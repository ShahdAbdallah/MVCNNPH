...
def add_velocity_acceleration_features(seq_2d):
    ...
    return np.concatenate([seq_2d, velocity, acceleration], axis=1)
...
def run_experiment(camera_mode):
    ...
    if camera_mode == "ALL_COMPLETE":
        folder_name = f"results_single_{TARGET_EXERCISE}_all_cameras_complete_velocity_acceleration_bs{BATCH_SIZE}"
        title_prefix = f"{TARGET_EXERCISE} All Cameras Complete"
        target_camera_text = "C0+C1+C2 complete only"
    else:
        folder_name = f"results_single_{TARGET_EXERCISE}_{camera_mode}_velocity_acceleration_bs{BATCH_SIZE}"
    ...
    print("\n=================================================")
    print(f"Running: {title_prefix} Velocity + Acceleration")
    ...
    with open(os.path.join(output_dir, "metrics.txt"), "w", encoding="utf-8") as f:
        f.write(f"===== Single-view {TARGET_EXERCISE} {target_camera_text} Velocity + Acceleration BS{BATCH_SIZE} Results =====\n")
