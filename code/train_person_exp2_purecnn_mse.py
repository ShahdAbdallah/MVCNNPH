...
OUTPUT_DIR = f"/mvdlph/shahd/MVCNNPH/results_by_person_exp3_mse_oversample_aug/len{TARGET_LEN}"
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("Experiment: EXP3 MSE + Velocity + Exercise Embedding + Oversampling Augmentation")
...
def load_split(split_name):
    ...
    for sample in segment_samples:
        ...
        grouped[group_key].append((sample["segment"], sample["x"], sample["y"], sample["file_name"]))
    ...
    return trial_samples, bad_files
...
def oversample_low_scores_with_augmentation(samples):
    ...
    for group_name, group in [("low", low), ("mid_low", mid_low)]:
        ...
        for i in range(needed):
            ...
            aug_sample = {
                "file_name": f"{base_sample['file_name']}_aug_{group_name}_{i}",
                "exercise": base_sample["exercise"],
                "person": base_sample["person"],
                "trial": base_sample["trial"],
                "camera": base_sample["camera"],
                "x": augment_sequence(base_sample["x"]),
                "y": float(base_sample["y"]),
                "is_augmented": 1
            }
    ...
    print("Original train samples:", len(original))
    print("Low score samples (<3):", len(low))
    print("Mid-low score samples [3,4):", len(mid_low))
    print("High score samples (>=4):", len(high))
    print("Augmented samples added:", len(augmented))
    print("Final train samples:", len(original) + len(augmented))

    summary = pd.DataFrame([
        {"group": "low_<3", "original_count": len(low), "target_count": target_count},
        {"group": "mid_low_3_to_4", "original_count": len(mid_low), "target_count": target_count},
        {"group": "high_>=4", "original_count": len(high), "target_count": target_count},
        {"group": "augmented_added", "original_count": len(augmented), "target_count": target_count},
        {"group": "final_train", "original_count": len(original) + len(augmented), "target_count": target_count},
    ])

    summary.to_csv(os.path.join(OUTPUT_DIR, "oversampling_summary.csv"), index=False)

    return original + augmented


fit_samples, bad_train = load_split("train")
val_samples, bad_valid = load_split("valid")
test_samples, bad_test = load_split("test")

fit_samples_original_count = len(fit_samples)
fit_samples = oversample_low_scores_with_augmentation(fit_samples)
fit_samples_after_aug_count = len(fit_samples)
...
print("\nFit samples after augmentation:", len(fit_samples))
...
with open(os.path.join(OUTPUT_DIR, "metrics.txt"), "w", encoding="utf-8") as f:
    f.write("===== EXP3 MSE + Velocity + Exercise Embedding + Oversampling Augmentation Results =====\n")
    ...
    f.write("Oversampling applied to train split only\n")
    f.write("Low-score groups = score < 3.0 and 3.0 <= score < 4.0\n")
    f.write("Augmentation = jitter, scaling, temporal shift, small rotation, mirroring\n")
    f.write(f"Original train samples = {fit_samples_original_count}\n")
    f.write(f"Train samples after augmentation = {fit_samples_after_aug_count}\n\n")
...
plt.title("Regression Loss (MSE) - Oversampling + Augmentation")
plt.xlabel("Epoch")
plt.ylabel("Loss")
...
plt.suptitle("RMSE & MAE — Train / Val / Test")
plt.tight_layout()
