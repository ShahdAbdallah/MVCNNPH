...
OUTPUT_DIR = f"/mvdlph/shahd/MVCNNPH/results_multiview_middle_fusion_all_except_E3_E7_E9_velocity_only/len{TARGET_LEN}"
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("Experiment: Multi-view Middle Fusion Velocity Only")
...
middle_fusion = Concatenate(axis=-1, name="middle_concat")(
...
with open(
    ...
    f.write("===== Multi-view Middle Fusion Velocity Only: All Exercises Except E3, E7, E9 =====\n\n")

    f.write(f"Target_len = {TARGET_LEN}\n")
    f.write(f"Epochs requested = {EPOCHS}\n")
    f.write(f"Epochs trained = {len(history.history['loss'])}\n")
    f.write(f"Batch size = {BATCH_SIZE}\n")
    f.write("Fusion type = Middle fusion\n")
    f.write("Fusion method = Concatenate feature maps after two CNN blocks per camera\n")
...
plt.suptitle("Middle Fusion RMSE & MAE")
plt.tight_layout()
plt.savefig(
