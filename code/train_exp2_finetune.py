...
OUTPUT_DIR = f"/mvdlph/shahd/MVCNNPH/results_exp2_stratified_finetune_lr/{CAMERA_TO_USE}_len{TARGET_LEN}"
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("Experiment: EXP2 Pure CNN + Stratified Validation + Fine LR")
...
print("\nFull stratification distribution:")
...
for sample in trial_samples:
    ...
    if key in train_keys:
        train_samples_all.append(sample)
    elif key in test_keys:
        test_samples.append(sample)

train_df_for_val = train_df.copy()
train_df_for_val["stratify_key"] = train_df_for_val["exercise"] + "_" + train_df_for_val["performance"]

val_strata_counts = train_df_for_val["stratify_key"].value_counts()
val_too_small = val_strata_counts[val_strata_counts < 2]

if len(val_too_small) > 0:
    print("\nWarning: Some train strata have less than 2 samples for validation split.")
    print(val_too_small)
    print("Falling back to performance-only validation stratification.")

    val_stratify_col = train_df_for_val["performance"]

    if (val_stratify_col.value_counts() < 2).any():
        print("Performance classes too small. Falling back to random validation split.")
        val_stratify_col = None
else:
    val_stratify_col = train_df_for_val["stratify_key"]
...
fit_keys = set(zip(fit_df["exercise"], fit_df["person"], fit_df["trial"]))
val_keys = set(zip(val_df["exercise"], val_df["person"], val_df["trial"]))
...
print("\nFit samples:", len(fit_samples))
print("Val samples:", len(val_samples))
print("Test samples:", len(test_samples))

train_df.to_csv(os.path.join(OUTPUT_DIR, "train_plus_val_trials.csv"), index=False)
fit_df.to_csv(os.path.join(OUTPUT_DIR, "fit_trials.csv"), index=False)
val_df.to_csv(os.path.join(OUTPUT_DIR, "validation_trials.csv"), index=False)
...
with open(os.path.join(OUTPUT_DIR, "split_summary.txt"), "w") as f:
    ...
    f.write(f"Fit trials = {len(fit_df)}\n")
    f.write(f"Validation trials = {len(val_df)}\n")
    f.write(f"Test trials = {len(test_df)}\n\n")

    f.write("Fit performance distribution:\n")
    f.write(str(fit_df["performance"].value_counts()))
    f.write("\n\nValidation performance distribution:\n")
    f.write(str(val_df["performance"].value_counts()))
    f.write("\n\nTest performance distribution:\n")
    f.write(str(test_df["performance"].value_counts()))

    f.write("\n\nFit exercise distribution:\n")
    f.write(str(fit_df["exercise"].value_counts().sort_index()))
    f.write("\n\nValidation exercise distribution:\n")
    f.write(str(val_df["exercise"].value_counts().sort_index()))
...
X_fit, y_fit, fit_meta = prepare_xy(fit_samples, TARGET_LEN)
X_val, y_val, val_meta = prepare_xy(val_samples, TARGET_LEN)
...
plt.title(f"EXP2 Fine LR Loss {CAMERA_TO_USE}")
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.legend()
...
plt.title(f"EXP2 Fine LR MAE {CAMERA_TO_USE}")
plt.xlabel("Epoch")
plt.ylabel("MAE")
plt.legend()
...
plt.axhline(y=test_loss, linestyle="--", label="Test Loss")
plt.title(f"EXP2 Fine LR Loss Raw {CAMERA_TO_USE}")
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.legend()
...
plt.axhline(y=test_mae_scaled, linestyle="--", label="Test MAE scaled")
plt.title(f"EXP2 Fine LR MAE Raw {CAMERA_TO_USE}")
plt.xlabel("Epoch")
plt.ylabel("MAE")
plt.legend()
...
plt.title(f"EXP2 Fine LR True vs Predicted {CAMERA_TO_USE}")
