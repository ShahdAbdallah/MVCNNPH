...
OUTPUT_DIR = f"/mvdlph/shahd/MVCNNPH/results_multiview_weighted_late_fusion_all_except_E3_E7_E9_velocity_only/len{TARGET_LEN}"
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("Experiment: Multi-view Weighted Late Fusion Velocity Only")
print("Included exercises: all except", sorted(EXCLUDED_EXERCISES))
print("Target length:", TARGET_LEN)
print("Fusion: Weighted Late Fusion")
...
merged = Concatenate(name="weighted_late_fusion_concat")(
    [weighted_c0, weighted_c1, weighted_c2]
)
...
with open(
    ...
    f.write("===== Multi-view Weighted Late Fusion Velocity Only: All Exercises Except E3, E7, E9 =====\n\n")

    f.write(f"Target_len = {TARGET_LEN}\n")
    f.write(f"Epochs requested = {EPOCHS}\n")
    f.write(f"Epochs trained = {len(history.history['loss'])}\n")
    f.write(f"Batch size = {BATCH_SIZE}\n")
    f.write("Fusion type = Weighted late fusion\n")
    f.write("Fusion method = multiply camera-specific vectors by fixed weights after full CNN branches, then concatenate\n")
    ...
    f.write("Dense layers after fusion = 64, 32\n")
...
plt.title("Weighted Late Fusion Loss")
plt.xlabel("Epoch")
plt.ylabel("Loss")
...
plt.suptitle("Weighted Late Fusion RMSE & MAE")
plt.tight_layout()
plt.savefig(
