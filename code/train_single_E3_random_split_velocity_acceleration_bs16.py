...
def safe_pcc(true, pred):
    ...
    try:
        value, pvalue = pearsonr(true, pred)
        return value, pvalue
    except Exception:
        return np.nan, np.nan


def load_all_e3_segments(camera_mode):
    segment_samples = []
    bad_files = []
    ...
    for split_name in ["train", "valid", "test"]:
        ...
        for file_name in sorted(os.listdir(split_dir)):
            ...
            if not match:
                bad_files.append((split_name, file_name, "filename pattern not matched"))
                continue
            ...
            if label_key not in score_map:
                bad_files.append((split_name, file_name, "missing label"))
                continue
            ...
            try:
                ...
                bad_files.append((split_name, file_name, str(e)))
                continue
            ...
            if (
                ...
                bad_files.append((split_name, file_name, str(keypoints_3d.shape)))
                continue
    ...
    return segment_samples, bad_files, skipped_not_target


def build_trial_samples(camera_mode):
    segment_samples, bad_files, skipped_not_target = load_all_e3_segments(camera_mode)
    ...
    for group_key, segs in grouped.items():
        ...
        for _, arr, _, _, _ in segs:
            if arr.ndim == 3 and arr.shape[1:] == (17, 3) and arr.shape[0] > 0:
                arrays.append(arr)
            else:
                bad_files.append((str(group_key), "bad segment shape during concat"))

        if len(arrays) == 0:
            bad_files.append((str(group_key), "no valid arrays"))
            continue
        ...
        trial_samples.append({
            ...
            "trial": trial,
            "camera": camera,
            "trial_key": f"{exercise}_{person}_{trial}",
...
def random_trial_split(samples):
    trial_keys = sorted(list(set([s["trial_key"] for s in samples])))
    ...
    train_keys = set(trial_keys[:n_train])
    val_keys = set(trial_keys[n_train:n_train + n_val])
    test_keys = set(trial_keys[n_train + n_val:])

    fit_samples = [s for s in samples if s["trial_key"] in train_keys]
    val_samples = [s for s in samples if s["trial_key"] in val_keys]
    test_samples = [s for s in samples if s["trial_key"] in test_keys]
    ...
    return fit_samples, val_samples, test_samples, pd.DataFrame(split_rows)
...
def add_velocity_acceleration_features(seq_2d):
    velocity = np.zeros_like(seq_2d, dtype=np.float32)
    velocity[1:] = seq_2d[1:] - seq_2d[:-1]

    acceleration = np.zeros_like(seq_2d, dtype=np.float32)
    acceleration[1:] = velocity[1:] - velocity[:-1]

    return np.concatenate(
        [seq_2d, velocity, acceleration],
        axis=1
    )
...
def prepare_xy(sample_list):
    ...
    for sample in sample_list:
        ...
        seq_2d = add_velocity_acceleration_features(seq_2d)
...
class RegressionMetricsCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        train_pcc_value, _ = safe_pcc(self.y_train_scaled, train_pred)
        val_pcc_value, _ = safe_pcc(self.y_val_scaled, val_pred)
...
def save_plots(
    ...
    plt.axhline(
        ...
        linestyle="-.",
        linewidth=2,
        label=f"Test RMSE={test_rmse_scaled:.4f}"
    )
    plt.title("RMSE")
    plt.xlabel("Epoch")
    plt.ylabel("RMSE")
    ...
    plt.axhline(
        ...
        linestyle="-.",
        linewidth=2,
        label=f"Test MAE={test_mae_scaled:.4f}"
    )
    plt.title("MAE")
    plt.xlabel("Epoch")
    plt.ylabel("MAE")
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "rmse_mae.png"),
        dpi=300,
        bbox_inches="tight"
    )
    plt.close()
...
def run_experiment(camera_mode):
    ...
    if camera_mode == "ALL_COMPLETE":
        folder_name = f"results_random_split_single_{TARGET_EXERCISE}_all_cameras_complete_velocity_acceleration_bs{BATCH_SIZE}"
        title_prefix = f"Random Split {TARGET_EXERCISE} All Cameras Complete"
        target_camera_text = "C0+C1+C2 complete only"
    else:
        folder_name = f"results_random_split_single_{TARGET_EXERCISE}_{camera_mode}_velocity_acceleration_bs{BATCH_SIZE}"
        title_prefix = f"Random Split {TARGET_EXERCISE} {camera_mode}"
    ...
    print("\n=================================================")
    print(f"Running: {title_prefix}")
    print("Output:", output_dir)
    print("=================================================\n")

    all_samples, bad_files, skipped_not_target, incomplete_trials = build_trial_samples(camera_mode)

    fit_samples, val_samples, test_samples, random_split_df = random_trial_split(all_samples)

    with open(os.path.join(output_dir, "bad_files.txt"), "w", encoding="utf-8") as f:
        for item in bad_files:
            f.write(str(item) + "\n")

    random_split_df.to_csv(
        os.path.join(output_dir, "random_trial_split.csv"),
        index=False
    )
    ...
    print("Skipped non-target files:", skipped_not_target)
    ...
    split_distribution_df.to_csv(
        os.path.join(output_dir, "split_distribution.csv"),
        index=False
    )
    ...
    person_summary_df.to_csv(
        os.path.join(output_dir, "person_split_summary.csv"),
        index=False
    )
    ...
    camera_summary_df.to_csv(
        os.path.join(output_dir, "camera_split_summary.csv"),
        index=False
    )
    ...
    y_fit_scaled = y_scaler.fit_transform(
        y_fit.reshape(-1, 1)
    ).flatten()
    ...
    worst_predictions_df = predictions_df.sort_values(
        "abs_error",
        ascending=False
    ).head(30)

    worst_predictions_df.to_csv(
        os.path.join(output_dir, "worst_30_predictions.csv"),
        index=False
    )
    ...
    per_person_df.to_csv(
        os.path.join(output_dir, "per_person_scores.csv"),
        index=False
    )
    ...
    for camera, group in predictions_df.groupby("camera"):
        ...
        camera_pcc, _ = safe_pcc(true, pred)
    ...
    per_camera_df.to_csv(
        os.path.join(output_dir, "per_camera_scores.csv"),
        index=False
    )

    with open(os.path.join(output_dir, "metrics.txt"), "w", encoding="utf-8") as f:
        f.write(f"===== Random Split Single-view {TARGET_EXERCISE} {target_camera_text} Velocity + Acceleration BS{BATCH_SIZE} Results =====\n")
        f.write(f"Target exercise = {TARGET_EXERCISE}\n")
        f.write(f"Target camera mode = {target_camera_text}\n")
        f.write("Split type = Random trial-level split\n")
        f.write("Original by-person folders were loaded together then randomly re-split\n")
        ...
        f.write("Added features = position + velocity + acceleration\n")
        ...
        f.write(f"Skipped non-target files = {skipped_not_target}\n")
