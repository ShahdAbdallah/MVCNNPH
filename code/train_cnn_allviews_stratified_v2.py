...
OUTPUT_DIR = f"/mvdlph/shahd/MVCNNPH/results_allviews_stratified_cnn_v2/allviews_len{TARGET_LEN}"
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("Experiment: Pure CNN v2 - All views stratified")
...
if len(too_small) > 0:
    print("\nSome strata are too small. Falling back to exercise-only stratification.")
    stratify_col = trial_df["exercise"]

    if (stratify_col.value_counts() < 2).any():
        print("Some exercises are too small. Falling back to random split.")
        stratify_col = None
else:
...
print("Train unique trials:", len(train_df))
print("Test unique trials :", len(test_df))
...
for camera, group in predictions_df.groupby("camera"):
    ...
    cam_r2 = r2_score(cam_true, cam_pred) if len(group) >= 2 else np.nan
...
with open(os.path.join(OUTPUT_DIR, "metrics.txt"), "w") as f:
    f.write("===== Pure CNN v2 - All Views Stratified Results =====\n")
...
plt.plot(history.history["val_loss"], alpha=0.35, label="Val Loss raw")
...
plt.plot(history.history["val_mae"], alpha=0.35, label="Val MAE raw")
plt.plot(smooth_curve(history.history["mae"]), label="Train MAE smooth")
plt.plot(smooth_curve(history.history["val_mae"]), label="Val MAE smooth")
plt.axhline(y=test_mae_scaled, linestyle="--", label="Test MAE scaled")
plt.title("Pure CNN v2 MAE")
plt.xlabel("Epoch")
plt.ylabel("MAE")
plt.legend()
...
plt.axhline(y=test_loss, linestyle="--", label="Test Loss")
plt.title("Pure CNN v2 Loss Raw")
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.legend()
...
plt.title("Pure CNN v2 True vs Predicted")
