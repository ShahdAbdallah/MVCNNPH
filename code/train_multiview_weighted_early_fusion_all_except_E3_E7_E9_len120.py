...
# Fixed camera weights for weighted early fusion.
# Here weights are applied to the preprocessed input features before concatenation.
W_C0 = 0.50
W_C1 = 0.25
W_C2 = 0.25

OUTPUT_DIR = f"/mvdlph/shahd/MVCNNPH/results_multiview_weighted_early_fusion_all_except_E3_E7_E9_velocity_only/len{TARGET_LEN}"
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("Experiment: Multi-view Weighted Early Fusion Velocity Only")
print("Included exercises: all except", sorted(EXCLUDED_EXERCISES))
print("Target length:", TARGET_LEN)
print("Fusion: Weighted Early Fusion")
...
def load_multiview_split(split_name):
    ...
    for trial_key, cam_dict in trial_camera_sequences.items():
        available_cameras = set(cam_dict.keys())

        if not REQUIRED_CAMERAS.issubset(available_cameras):
            incomplete_trials.append({
                "split": split_name,
                "exercise": trial_key[0],
                "person": trial_key[1],
                "trial": trial_key[2],
                "available": ",".join(sorted(available_cameras)),
                "missing": ",".join(sorted(REQUIRED_CAMERAS - available_cameras))
            })
            continue
...
def prepare_xy(samples):
    ...
    for sample in samples:
        ...
        # Apply weights to preprocessed per-camera input features BEFORE CNN.
...
exercise_summary_df.to_csv(
    os.path.join(OUTPUT_DIR, "exercise_split_summary.csv"),
    index=False
)
...
seq_input = Input(shape=(TARGET_LEN, feature_dim), name="weighted_early_fusion_input")
...
per_exercise_df.to_csv(
    os.path.join(OUTPUT_DIR, "per_exercise_scores.csv"),
    index=False
)
...
with open(
    ...
    f.write("===== Multi-view Weighted Early Fusion Velocity Only: All Exercises Except E3, E7, E9 =====\n\n")

    f.write(f"Target_len = {TARGET_LEN}\n")
    f.write(f"Epochs requested = {EPOCHS}\n")
    f.write(f"Epochs trained = {len(history.history['loss'])}\n")
    f.write(f"Batch size = {BATCH_SIZE}\n")
    f.write("Fusion type = Weighted early fusion\n")
    f.write("Fusion method = multiply preprocessed camera inputs by fixed weights before CNN, then concatenate along feature dimension\n")
    ...
    f.write("Weighted fused input features = 306\n")
    f.write("Loss = MSE\n")
...
plt.title("Weighted Early Fusion Loss")
plt.xlabel("Epoch")
plt.ylabel("Loss")
...
plt.suptitle("Weighted Early Fusion RMSE & MAE")
plt.tight_layout()
plt.savefig(
