...
OUTPUT_DIR = f"/mvdlph/shahd/MVCNNPH/results_stratified_cnn/{CAMERA}_len{TARGET_LEN}"
...
# =========================================================
# Load valid segments for one camera
# =========================================================
...
print("Grouped trials:", len(trial_samples))
...
# =========================================================
# Stratified split: exercise + performance level
# =========================================================
...
if len(too_small) > 0:
    print("\nWarning: Some strata have less than 2 samples.")
    print(too_small)
...
print("\nTrain trials:", len(train_samples))
print("Test trials :", len(test_samples))
...
# =========================================================
# Train for fixed 100 epochs, no EarlyStopping
# =========================================================
history = model.fit(
    X_train,
    y_train_scaled,
...
with open(os.path.join(OUTPUT_DIR, "metrics.txt"), "w") as f:
    f.write("===== Stratified Pure CNN Results =====\n")
    ...
    f.write(f"Train trials = {len(train_samples)}\n")
    f.write(f"Test trials  = {len(test_samples)}\n")
...
plt.title(f"Stratified Pure CNN Loss ({CAMERA})")
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.legend()
...
plt.title(f"Stratified Pure CNN MAE ({CAMERA})")
plt.xlabel("Epoch")
plt.ylabel("MAE")
plt.legend()
...
plt.title(f"Stratified Pure CNN True vs Predicted ({CAMERA})")
