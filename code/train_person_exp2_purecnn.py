...
OUTPUT_DIR = f"/mvdlph/shahd/MVCNNPH/results_by_person_exp3_velocity_embedding_light_weighted/len{TARGET_LEN}"
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("Experiment: EXP3 Velocity + Exercise Embedding + Light Low-Score Weighting")
...
def load_split(split_name):
    ...
    for sample in segment_samples:
        ...
        grouped[group_key].append((sample["segment"], sample["x"], sample["y"]))
...
class RegressionMetricsCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        train_pred = self.model.predict(
            [self.X_train, self.X_train_ex],
            verbose=0
        ).flatten()

        val_pred = self.model.predict(
            [self.X_val, self.X_val_ex],
            verbose=0
        ).flatten()
...
with open(os.path.join(OUTPUT_DIR, "metrics.txt"), "w", encoding="utf-8") as f:
    f.write("===== EXP3 Velocity + Exercise Embedding + Light Low-Score Weighting Results =====\n")
