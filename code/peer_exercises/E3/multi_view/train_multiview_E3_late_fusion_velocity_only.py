...
OUTPUT_DIR = f"/mvdlph/shahd/MVCNNPH/results_multiview_E3_late_fusion_velocity_only/len{TARGET_LEN}"
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("Experiment: Multi-view E3 Late Fusion Velocity Only")
print("Target exercise:", TARGET_EXERCISE)
print("Fusion: Late fusion")
...
def load_multiview_split(split_name):
    ...
    for trial_key, cam_dict in trial_camera_sequences.items():
        ...
        if not REQUIRED_CAMERAS.issubset(available_cameras):
            incomplete_trials.append({
                ...
                "person": person,
                "trial": trial,
                "available_cameras": ",".join(sorted(available_cameras)),
                "missing_cameras": ",".join(sorted(REQUIRED_CAMERAS - available_cameras))
            })
            continue
    ...
    return multiview_samples, bad_files, incomplete_trials, skipped_not_e0
...
print("Skipped non-E3 files:", skipped_train + skipped_valid + skipped_test)
...
print("X0_fit shape:", X0_fit.shape)
print("X1_fit shape:", X1_fit.shape)
print("X2_fit shape:", X2_fit.shape)
...
class RegressionMetricsCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        train_pred = self.model.predict(self.X_train_list, verbose=0).flatten()
        val_pred = self.model.predict(self.X_val_list, verbose=0).flatten()

        self.train_r2.append(r2_score(self.y_train_scaled, train_pred))
        self.val_r2.append(r2_score(self.y_val_scaled, val_pred))

        self.train_pcc.append(
            pearsonr(self.y_train_scaled, train_pred)[0] if len(self.y_train_scaled) > 1 else np.nan
        )
        self.val_pcc.append(
            pearsonr(self.y_val_scaled, val_pred)[0] if len(self.y_val_scaled) > 1 else np.nan
        )
...
with open(os.path.join(OUTPUT_DIR, "metrics.txt"), "w", encoding="utf-8") as f:
    f.write("===== Multi-view E3 Late Fusion Velocity Only Results =====\n")
    f.write(f"Target exercise = {TARGET_EXERCISE}\n")
    f.write("Fusion type = Late fusion\n")
    ...
    f.write("Each camera branch filters = 16, 24, 24, 32\n")
    f.write("Dropout = 0.10\n")
    f.write("Added features = velocity only\n")
    f.write("Input per view = 102\n")
    f.write("Fusion after camera-specific CNN feature extraction\n")
    ...
    f.write(f"Skipped non-E3 files = {skipped_train + skipped_valid + skipped_test}\n\n")
...
plt.title("E3 Late Fusion Loss (MSE)")
plt.xlabel("Epoch")
plt.ylabel("Loss")
...
plt.suptitle("E3 Late Fusion RMSE & MAE")
plt.tight_layout()
...
plt.title("E3 Late Fusion True vs Predicted")
plt.grid(True, alpha=0.25)
plt.tight_layout()
