...
OUTPUT_DIR = f"/mvdlph/shahd/MVCNNPH/results_single_E0_C0_velocity_only/len{TARGET_LEN}"
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("Experiment: Single-view E0 C0 Velocity Only")
...
fit_samples, bad_train, skipped_train = load_split("train")
val_samples, bad_valid, skipped_valid = load_split("valid")
test_samples, bad_test, skipped_test = load_split("test")
...
with open(os.path.join(OUTPUT_DIR, "metrics.txt"), "w", encoding="utf-8") as f:
    f.write("===== Single-view E0 C0 Velocity Only Results =====\n")
    f.write(f"Target exercise = {TARGET_EXERCISE}\n")
    f.write(f"Target camera = {TARGET_CAMERA}\n")
...
plt.title("E0 C0 Regression Loss (MSE)")
plt.xlabel("Epoch")
plt.ylabel("Loss")
...
plt.title("E0 C0 True vs Predicted")
plt.grid(True, alpha=0.25)
plt.tight_layout()
