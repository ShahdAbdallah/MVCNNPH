...
def compute_metrics(y_true, y_pred):
    pcc, pval = safe_pcc(y_true, y_pred)
    return {
...
def load_split(split_name, camera_mode):
    ...
    if camera_mode == "ALL_COMPLETE":
        ...
        for s in segment_samples:
            trial_to_cameras[(s["exercise"], s["person"], s["trial"])].add(s["camera"])
        valid_trials = set()
        ...
        segment_samples = [s for s in segment_samples if (s["exercise"], s["person"], s["trial"]) in valid_trials]

    grouped = defaultdict(list)
    for s in segment_samples:
        key = (s["exercise"], s["person"], s["trial"], s["camera"])
        grouped[key].append((s["segment"], s["x"], s["y"], s["file_name"]))
    ...
    for group_key, segs in grouped.items():
        ...
        for _, arr, _, _ in segs:
            if arr.ndim == 3 and arr.shape[1:] == (17, 3) and arr.shape[0] > 0:
                ...
                bad_files.append((str(group_key), f"invalid segment shape {arr.shape}"))
        if not arrays:
            bad_files.append((str(group_key), "no valid arrays after filtering"))
            continue
        try:
            full_sequence = np.concatenate(arrays, axis=0)
        except Exception as e:
            bad_files.append((str(group_key), f"concatenate error: {e}"))
            continue
    ...
    return trial_samples, bad_files, skipped_not_target, incomplete_trials, label_debug_rows
...
def samples_to_df(samples, split_name):
    return pd.DataFrame([{
        ...
        "num_segments": s.get("num_segments", np.nan),
        "full_sequence_shape": s.get("full_sequence_shape", ""),
    } for s in samples])
...
def prepare_xy(sample_list):
    ...
    for sample in sample_list:
        seq_3d = center_skeleton(sample["x"].copy())
        ...
        meta.append({"exercise": sample["exercise"], "person": sample["person"], "trial": sample["trial"], "camera": sample["camera"]})
...
class RegressionMetricsCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        self.train_pcc.append(safe_pcc(self.y_train_scaled, train_pred)[0])
        self.val_pcc.append(safe_pcc(self.y_val_scaled, val_pred)[0])
...
def save_plots(history, epoch_metrics, test_loss, test_mae_scaled, test_rmse_scaled, r2, pcc, y_test, y_pred, output_dir, title_prefix):
    ...
    loss_ymin, loss_ymax = get_ylim(loss_values + val_loss_values + [test_loss])
    ...
    plt.xlabel("Epoch"); plt.ylabel("Loss"); plt.ylim(loss_ymin, loss_ymax)
    ...
    plt.title("RMSE"); plt.xlabel("Epoch"); plt.ylabel("RMSE"); plt.ylim(rmse_ymin, rmse_ymax); plt.grid(True, alpha=0.25); plt.legend()
    plt.subplot(1, 2, 2)
    ...
    plt.title("MAE"); plt.xlabel("Epoch"); plt.ylabel("MAE"); plt.ylim(mae_ymin, mae_ymax); plt.grid(True, alpha=0.25); plt.legend()
    plt.suptitle(f"{title_prefix} RMSE & MAE")
    plt.tight_layout(); plt.savefig(os.path.join(output_dir, "rmse_mae.png"), dpi=300, bbox_inches="tight"); plt.close()

    r2_ymin, r2_ymax = get_ylim(epoch_metrics.train_r2 + epoch_metrics.val_r2 + [r2, 0, 1], pad_ratio=0.10)
    plt.figure(figsize=(9, 5))
    ...
    plt.title(f"{title_prefix} R²"); plt.xlabel("Epoch"); plt.ylabel("R²"); plt.ylim(r2_ymin, r2_ymax)
    plt.grid(True, alpha=0.25); plt.legend(); plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "r2_curve.png"), dpi=300, bbox_inches="tight"); plt.close()

    pcc_ymin, pcc_ymax = get_ylim(epoch_metrics.train_pcc + epoch_metrics.val_pcc + [pcc, 0, 1], pad_ratio=0.10)
    plt.figure(figsize=(9, 5))
    ...
    plt.title(f"{title_prefix} PCC"); plt.xlabel("Epoch"); plt.ylabel("PCC"); plt.ylim(pcc_ymin, pcc_ymax)
    plt.grid(True, alpha=0.25); plt.legend(); plt.tight_layout()
...
def create_prediction_df(meta_list, y_true, y_pred, split_name):
    return pd.DataFrame([{
        ...
        "predicted_score": pv,
        "abs_error": abs(tv - pv),
    } for m, tv, pv in zip(meta_list, y_true, y_pred)])


def per_group_scores(df, group_col, out_name):
    rows = []
    for key, group in df.groupby(group_col):
        true, pred = group["true_score"].values, group["predicted_score"].values
        pcc, _ = safe_pcc(true, pred)
        rows.append({
            group_col: key,
...
def run_experiment(camera_mode):
    reset_seed()
    if camera_mode == "ALL_COMPLETE":
        folder_name = f"results_single_{TARGET_EXERCISE}_all_cameras_complete_velocity_only_len120_with_train_val_test"
        title_prefix = f"{TARGET_EXERCISE} All Cameras Complete"
        target_camera_text = "C0+C1+C2 complete only"
    else:
        folder_name = f"results_single_{TARGET_EXERCISE}_{camera_mode}_velocity_only_len120_with_train_val_test"
    ...
    fit_samples, bad_train, skipped_train, incomplete_train, debug_train = load_split("train", camera_mode)
    val_samples, bad_valid, skipped_valid, incomplete_valid, debug_valid = load_split("valid", camera_mode)
    test_samples, bad_test, skipped_test, incomplete_test, debug_test = load_split("test", camera_mode)
    bad_files = bad_train + bad_valid + bad_test
    incomplete_trials = incomplete_train + incomplete_valid + incomplete_test
    label_debug_rows = debug_train + debug_valid + debug_test

    with open(os.path.join(output_dir, "bad_files.txt"), "w", encoding="utf-8") as f:
        for item in bad_files:
            f.write(str(item) + "\n")
    pd.DataFrame(label_debug_rows).to_csv(os.path.join(output_dir, "label_camera_debug_first_files.csv"), index=False)
    if camera_mode == "ALL_COMPLETE":
    ...
    print("Train camera counts:", Counter([s["camera"] for s in fit_samples]))
    print("Validation camera counts:", Counter([s["camera"] for s in val_samples]))
    print("Test camera counts:", Counter([s["camera"] for s in test_samples]))
    ...
    person_summary_df = split_distribution_df.groupby(["split", "person"]).agg(
        num_samples=("score", "count"), mean_score=("score", "mean"), min_score=("score", "min"), max_score=("score", "max"),
        good_count=("performance", lambda x: (x == "good").sum()), bad_count=("performance", lambda x: (x == "bad").sum())
    ).reset_index()
    ...
    camera_summary_df = split_distribution_df.groupby(["split", "camera"]).agg(
        num_samples=("score", "count"), mean_score=("score", "mean"),
        good_count=("performance", lambda x: (x == "good").sum()), bad_count=("performance", lambda x: (x == "bad").sum())
    ).reset_index()
    ...
    model = build_model(X_fit.shape[2])
    model.summary()
    ...
    history = model.fit(X_fit, y_fit_scaled, validation_data=(X_val, y_val_scaled), epochs=EPOCHS, batch_size=BATCH_SIZE, callbacks=callbacks, shuffle=True, verbose=1)
    ...
    train_metrics = compute_metrics(y_fit, train_pred)
    ...
    per_person_df = per_group_scores(test_predictions_df, "person", "per_person_scores.csv")
    per_camera_df = per_group_scores(test_predictions_df, "camera", "per_camera_scores.csv")
    ...
    with open(os.path.join(output_dir, "metrics.txt"), "w", encoding="utf-8") as f:
        ...
        f.write("Loss = MSE\nLearning rate = 2e-5\nFilters = 16, 24, 24, 32\nDropout = 0.10\n")
        f.write("Added features = velocity only\nExercise embedding = No\nOversampling = No\nReplacement = No\nAugmentation = No\n")
        f.write("Extra checks = label-camera debug CSV, train/validation/test metrics, camera counts\n")
        if camera_mode == "ALL_COMPLETE":
        ...
        for split_name, metrics, loss, mae_s, rmse_s in [
            ("Train", train_metrics, train_loss, train_mae_scaled, train_rmse_scaled),
            ("Validation", val_metrics, val_loss, val_mae_scaled, val_rmse_scaled),
            ("Test", test_metrics, test_loss, test_mae_scaled, test_rmse_scaled),
        ]:
            f.write(f"===== {split_name} Metrics =====\n")
            f.write(f"{split_name} MAE = {metrics['MAE']}\n")
            f.write(f"{split_name} RMSE = {metrics['RMSE']}\n")
            f.write(f"{split_name} R2 = {metrics['R2']}\n")
            f.write(f"{split_name} PCC = {metrics['PCC']}\n")
            f.write(f"{split_name} PCC p-value = {metrics['PCC_pvalue']}\n")
            f.write(f"{split_name} Loss scaled = {loss}\n")
            f.write(f"{split_name} MAE scaled = {mae_s}\n")
            f.write(f"{split_name} RMSE scaled = {rmse_s}\n\n")
        ...
        f.write("===== Camera Counts =====\n")
        f.write(f"Train camera counts = {dict(Counter([s['camera'] for s in fit_samples]))}\n")
        f.write(f"Validation camera counts = {dict(Counter([s['camera'] for s in val_samples]))}\n")
        f.write(f"Test camera counts = {dict(Counter([s['camera'] for s in test_samples]))}\n\n")
        f.write("===== Split Person Summary =====\n" + person_summary_df.to_string(index=False) + "\n\n")
        f.write("===== Camera Split Summary =====\n" + camera_summary_df.to_string(index=False) + "\n\n")
        f.write("===== Per-person test scores =====\n" + per_person_df.to_string(index=False) + "\n\n")
        f.write("===== Per-camera test scores =====\n" + per_camera_df.to_string(index=False) + "\n\n")
        f.write("===== Worst 30 test predictions =====\n" + worst_predictions_df.to_string(index=False))

    np.savez(
        os.path.join(output_dir, "plot_data.npz"),
        train_loss=np.array(history.history["loss"]), val_loss=np.array(history.history["val_loss"]),
        train_mae=np.array(history.history["mae"]), val_mae=np.array(history.history["val_mae"]),
        train_rmse=np.array(history.history["rmse"]), val_rmse=np.array(history.history["val_rmse"]),
        train_r2=np.array(epoch_metrics.train_r2), val_r2=np.array(epoch_metrics.val_r2),
        train_pcc=np.array(epoch_metrics.train_pcc), val_pcc=np.array(epoch_metrics.val_pcc),
        final_train_loss=np.array([train_loss]), final_val_loss=np.array([val_loss]), test_loss=np.array([test_loss]),
        train_mae_original=np.array([train_metrics["MAE"]]), val_mae_original=np.array([val_metrics["MAE"]]), test_mae_original=np.array([test_metrics["MAE"]]),
        train_rmse_original=np.array([train_metrics["RMSE"]]), val_rmse_original=np.array([val_metrics["RMSE"]]), test_rmse_original=np.array([test_metrics["RMSE"]]),
        train_r2_original=np.array([train_metrics["R2"]]), val_r2_original=np.array([val_metrics["R2"]]), test_r2_original=np.array([test_metrics["R2"]]),
        y_train=y_fit, y_train_pred=train_pred, y_val=y_val, y_val_pred=val_pred, y_test=y_test, y_pred=test_pred,
    )

    save_plots(history, epoch_metrics, test_loss, test_mae_scaled, test_rmse_scaled, test_metrics["R2"], test_metrics["PCC"], y_test, test_pred, output_dir, title_prefix)

    print("\nSaved files in:", output_dir)
    for name in [
        "metrics.txt", "train_validation_test_metrics.csv", "label_camera_debug_first_files.csv", "training_log.txt",
        "train_predictions.csv", "validation_predictions.csv", "predictions.csv", "all_split_predictions.csv",
        "worst_30_predictions.csv", "per_person_scores.csv", "per_camera_scores.csv", "split_distribution.csv",
        "person_split_summary.csv", "camera_split_summary.csv", "plot_data.npz", "loss_curve.png", "rmse_mae.png",
        "r2_curve.png", "pcc_curve.png", "true_vs_predicted.png", "images.tar.gz", "bad_files.txt", "best_model.keras",
    ]:
        print(name)
    if camera_mode == "ALL_COMPLETE":
